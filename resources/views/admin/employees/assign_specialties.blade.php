...
@section('content')
    <div class="container mx-auto p-4">
        <h1 class="text-2xl font-bold mb-4">{{ __('Assign Specialties to') }} {{ $employee->name }}</h1>
        <form action="{{ route('employees.store.specialties', $employee->id) }}" method="POST">
            @csrf
            <div class="mb-4">
                ...
                <select multiple id="specialties" name="specialties[]" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                    @foreach($specialties as $specialty)
                        <option value="{{ $specialty->id }}" {{ $employee->specialties->contains($specialty->id) ? 'selected' : '' }}>{{ $specialty->name }}</option>
                    @endforeach
                </select>
            </div>
