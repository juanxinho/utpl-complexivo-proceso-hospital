<form id="filterForm" method="GET" action="{{ route('admin.specialties.index') }}"
    ...
    <div class="relative">
        <div class="flex items-center">
            <select
                ...
                <option value="0" {{ request('statusFilter') == '0' ? 'selected' : '' }}>{{ __('Inactive') }}</option>
                <option value="1" {{ request('statusFilter') == '1' ? 'selected' : '' }}>{{ __('Active') }}</option>
            </select>
        </div>
    </div>
